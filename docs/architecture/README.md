...
| [0024](ADR-0024-s8c1-marketplace-connector-authorization-foundation.md) | S8C.1 Marketplace Connector and Authorization Foundation | Accepted — approved and frozen; implemented in `01e5d6a` (S8C.1 COMPLETE) | G-01..G-08 and RT-01..RT-03: bound sessions, one PostgreSQL operation arbiter, conservative duplicate identity handling, new secret generation after store loss, S8B upgrade and test-only fake; compliance checks in ADR |
| [0025](ADR-0025-s8c2-mercado-livre-listing-read-integration.md) | S8C.2 Mercado Livre Listing Read Integration and Normalized Observations | Accepted — approved and frozen; implementation not started | S8C.1 defects D-01..D-03 first; real ML adapter on the S8C.1 port with NOT_SENT only before `SendAsync` (Astra G01_CLOSED); G-08 HTTP layer; provider-neutral listing reader; lease-token-fenced runs with durable run items and FAILED_ONLY retry runs; explicit SKU mappings as sole auto-link authority; item-derived run outcomes and listing-sync health; no quantity, writes, webhooks or polling |
