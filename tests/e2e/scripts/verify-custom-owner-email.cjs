...
const { acquireE2eRunLock, dropE2eDatabaseIfExists, releaseE2eRunLock, runE2ePsql } = require('../e2e-env.cjs')

const CUSTOM_OWNER_EMAIL = 'custom-owner@example.test'
const CUSTOM_DATABASE = 'verce_e2e_custom_owner_check'

/**
 * H-03/§17: the ENTIRE destructive lifecycle — pre-drop, the reentrant Playwright child that
 * provisions the database, verification queries, and post-drop — runs inside one E2E run-lock
 * acquisition, not merely the individual DROP/CREATE calls (which now refuse to run unlocked
 * anyway; see e2e-env.cjs). Acquiring the lock here also sets the ambient
 * `VERCE_E2E_RUN_LOCK_HOLDER` marker on `process.env`, which the spawned `npx playwright test`
 * child inherits and authenticates against (the SAME reentrancy mechanism playwright.config.ts's
 * own worker processes already rely on) — it becomes a lawful descendant of this ownership rather
 * than a second, competing acquisition.
 */
function main() {
  const lock = acquireE2eRunLock()
  try {
    ...
    try {
      ...
      const ownerExists = runE2ePsql(CUSTOM_DATABASE, ['-tAc', `SELECT 1 FROM platform."user" WHERE normalized_email = '${CUSTOM_OWNER_EMAIL.toUpperCase()}'`])
      if (ownerExists !== '1') throw new Error(`Owner bootstrap did not create ${CUSTOM_OWNER_EMAIL} in ${CUSTOM_DATABASE}`)

      const operatorLinkedToCustomOwner = runE2ePsql(CUSTOM_DATABASE, ['-tAc', `SELECT 1 FROM platform."user" op
