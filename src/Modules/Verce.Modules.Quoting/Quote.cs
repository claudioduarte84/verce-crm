...
public sealed class Quote : AggregateRoot
{
    ...
    public Quote(int numberSequence, DateOnly numberDate, CustomerSnapshotInput customer, Guid salesChannelId,
        IReadOnlyList<QuoteItemSnapshot> items, int validityDays, Guid? actorUserId, Guid correlationId, DateTimeOffset now)
    {
        ...
        var firstRevision = new QuoteRevision(Id, 1, customer, salesChannelId, items, validityDays, numberDate, now);
    ...
    public QuoteRevision ConstructNextRevision(CustomerSnapshotInput customer, Guid salesChannelId,
        IReadOnlyList<QuoteItemSnapshot> items, int validityDays, DateOnly organizationToday,
        Guid? actorUserId, Guid correlationId, DateTimeOffset now)
    {
        var current = CurrentRevision;
        var next = new QuoteRevision(Id, current.RevisionIndex + 1, customer, salesChannelId, items, validityDays, organizationToday, now)
        {
...
public sealed class QuoteRevision : Entity, IOwnedBy<Quote>
{
    ...
    internal QuoteRevision(Guid quoteId, int revisionIndex, CustomerSnapshotInput customer, Guid salesChannelId,
        IReadOnlyList<QuoteItemSnapshot> items, int validityDays, DateOnly organizationToday, DateTimeOffset now)
    {
